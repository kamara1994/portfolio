...
const bootLines = [
  ...
  { text: 'SPLUNK SIEM → ONLINE',             delay: 300,  color: '#00f5d4' },
  { text: 'PALO ALTO FIREWALL → ACTIVE',      delay: 600,  color: '#00f5d4' },
  { text: 'N8N ORCHESTRATION → RUNNING',      delay: 900,  color: '#00f5d4' },
  { text: 'CLAUDE LLM API → CONNECTED',       delay: 1200, color: '#818cf8' },
  { text: 'ALL SYSTEMS NOMINAL',              delay: 1600, color: '#39ff14' },
  { text: 'WELCOME, OPERATOR.',               delay: 1900, color: '#ffffff' },
]
...
export default function BootScreen({ onComplete }: BootScreenProps) {
  ...
  useEffect(() => {
    ...
    if (hasBooted) { setSkip(true); onComplete(); return }
      ...
      {!exit && (
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-xl px-6">
            ...
            >
              <div className="font-orbitron text-5xl font-black text-transparent bg-clip-text mb-2"
            ...
            <div className="terminal-box w-full mb-6">
              <div className="terminal-bar">
                ...
                <span className="font-mono text-[10px] text-muted ml-2 tracking-wider">jak@blue-soc — boot</span>
              </div>
              <div className="p-5 font-mono text-[12px] leading-7 min-h-[180px]">
            ...
            <div className="w-full">
              ...
              <div className="w-full h-1 bg-[rgba(0,212,255,0.1)] relative overflow-hidden">
                <motion.div className="h-full"
                  style={{ background: 'linear-gradient(90deg, #00d4ff, #00f5d4)', boxShadow: '0 0 10px rgba(0,212,255,0.8)' }}
              ...
              {done && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  className="text-center mt-3 font-mono text-[11px] tracking-[4px] uppercase"
                  style={{ color: '#39ff14' }}>
                  SYSTEM READY
                </motion.div>
              )}
          ...
          <button onClick={handleSkip}
            className="absolute bottom-8 right-8 font-mono text-[9px] text-muted hover:text-cyan transition-colors tracking-[2px] uppercase border border-[rgba(0,212,255,0.15)] px-3 py-1.5 hover:border-cyan">
            Skip →
          </button>
        </motion.div>
      )}
